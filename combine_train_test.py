...
if False:
    ...
    with open('/home/andy/ipl/dd3d/outputs/2023-10-17/23-22-15/inference/final/kradar_train_good_v3/bbox3d_predictions.json') as f:
        train_result = json.load(f)
    with open('/home/andy/Downloads/tmp_dd3d/bbox3d_predictions.json') as f:
    ...
    for box in result:
        ...
        z += 0.7
        x -= 0.1
        box['box3d'] = [z, -x, -y, L, W, H, float(r+np.pi/2)]
        box['quat'] = quat
...
# def nms_filter_cruw(pred_path, prediction_format):
    ...
    save_file_name = f'dd3d_dla34_3dnms_{iou_threshold}.json'
    ...
    with open('kradar_dla34_3dnms.pkl', 'wb') as f:
...
with open('kradar_dla34_3dnms.pkl', 'rb') as f:
    pred = pickle.load(f)
...
for k, v in pred.items():
    ...
    for i in range(len(v['box3d'])):
        ...
        frame_obj['psr']['position'] = {'x': x + float(tr_LR_tvec[0, 0]), 'y': y + float(tr_LR_tvec[0, 1]), 'z': z + float(tr_LR_tvec[0, 2])}
